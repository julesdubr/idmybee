...
app/                    Applications Streamlit
  setup_dataset.py         -> préparer/nettoyer un jeu de données jusqu'à l'export (scénario 1,
                              outil 1/3) -- les deux étapes de validation (recadrage, landmarks)
                              sont des checkpoints humains dans cette appli. Ni train_model.py ni
                              predict_dataset.py ne relancent le landmarking : les deux pointent
                              vers un jeu de données déjà préparé/exporté par cet outil.
  train_model.py           -> construire une forme de référence GPA et/ou entraîner un modèle
                              GPA-PCA-LDA sur un jeu de données déjà préparé (scénario 1, outil 2/3)
  predict_dataset.py       -> classifier un jeu de données déjà préparé avec un modèle existant
                              (scénario 1, outil 3/3)
...
data/                   Jeux de données utilisés/produits par le pipeline, seulement -- rien d'autre
  ...
  analysis/               Sorties d'ANOVA/analyse de variance (indépendant de tout modèle)

models/                 Modèles entraînés (LDA, UNet, YOLO-OBB) -- au niveau racine, pas dans data/
  lda/<model_name>/       un dossier par modèle LDA -- nommé d'après --model-name (slugifié), avec
                           un suffixe _v2/_v3/... automatique en cas de nom déjà pris -- voir
                           `core.run_io.resolve_model_slug` et l'étape 4 du scénario 1 plus bas
  unet_landmarks/, yolon_obb/   poids des détecteurs/du placement de landmarks

references/             Forme de référence GPA figée (`.tps`, voir `landmarks/build_reference.py`
                         et app/train_model.py), gabarits de landmarks -- au niveau racine

Il n'y a plus de dossier `data/identification/` séparé : le mapping
`inv_id_mapping.csv` (frozen original_id -> inv_id) et, quand construit
depuis l'appli plutôt que fourni à la main, `collection_origin_codes.csv`
vivent maintenant à l'intérieur du jeu de données produit par l'étape 1/2
elle-même (voir `tools.ingestion.prepare_dataset` et
`manifest/origin_table.py`).
...
- les photos brutes (`data/*/collection`, `data/*/terrain`... — dossiers
  `images/`) ;
- les poids des modèles déjà entraînés : `models/unet_landmarks/*/weights.pt`
  et `models/yolon_obb/best.pt` (nécessaires pour le scénario 2).

En revanche, le modèle de classification déjà entraîné
(`models/lda/species_collection/train/model.joblib`) et les petits
...
- **Avec l'interface graphique**, en trois outils Streamlit distincts,
  chacun couvrant une préoccupation cohérente (voir "Interface graphique"
  plus bas pour le détail) :
  - `streamlit run app/setup_dataset.py` — un assistant pas-à-pas qui
    enchaîne préparation -> détection/recadrage -> landmarks -> export,
    avec en plus deux **points de contrôle visuels** (recadrage, puis
    placement des landmarks) où l'on peut vérifier et corriger à la main
    le statut automatique (OK/SUSPECT/FAILED) de chaque photo avant de
    continuer. C'est le seul des trois outils qui relance le landmarking.
  - `streamlit run app/train_model.py` — construit une forme de référence
    GPA et/ou entraîne un modèle sur un jeu de données déjà préparé par
    l'outil précédent.
  - `streamlit run app/predict_dataset.py` — classifie un jeu de données
    déjà préparé avec un modèle existant.

  Recommandé pour un premier passage sur un nouveau jeu de données, où
  l'on veut *voir* ce qui se passe avant de faire confiance au modèle qui
  en sortira.
...
Une source y est juste un dossier de photos + un CSV d'identification --
pas de fichier `roots.json` séparé à préparer comme pour `ingest_raw.py`
ci-dessus : le scan brut de chaque source est un sous-produit jetable de
sa préparation (écrit dans un sous-dossier caché à l'intérieur même du
dossier de photos, effacé une fois la source préparée, sauf
`"keep_raw_manifest": true` dans la config) :
...
python -m tools.pipeline.train_dataset data/MonJeuDeDonnees \
    ...
    --reference references/shapes/Bombus_CR_19LM.tps \
    --model-name "Identification bourdons (collection)"
```

Ce script enchaîne automatiquement détection -> recadrage -> landmarks ->
renumérotation -> export -> entraînement GPA-PCA-LDA. `--reference` pointe
un fichier `.tps` simple (une seule forme, voir `landmarks/build_reference.py`
et `app/train_model.py` pour en construire un) -- le nombre de landmarks
attendu se lit directement dans ce fichier, plus besoin de le coder dans
son nom. Résultat : `models/lda/<nom_du_modèle>/train/model.joblib`, plus
...
et du nom du dataset. Ce nom (slugifié) nomme maintenant aussi le dossier
de sortie sous `models/lda/` : un nom déjà pris reçoit automatiquement un
suffixe `_v2`/`_v3`/... plutôt que d'écraser le run précédent (voir
`core.run_io.resolve_model_slug`) -- chaque entraînement obtient donc son
propre dossier, même à paramètres identiques. À défaut de `--model-name`,
l'id technique (`level_dataset[_appareils][_source]`) sert de nom de base
et suit la même règle de versionnage. Le nom retenu est repris tel quel
dans les listes déroulantes de modèle, aussi bien dans `app/single_image.py`
que dans `app/train_model.py`/`app/predict_dataset.py` (voir
`core.run_io.model_display_name`).

**5. Prédire sur un autre jeu de données (évaluation).**

```bash
python -m tools.pipeline.predict_dataset data/UnAutreJeuDeDonnees \
    --model models/lda/Identification-bourdons-collection/train/model.joblib \
    --unet-model models/unet_landmarks/2026-08-29_131929/weights.pt \
    --reference references/shapes/Bombus_CR_19LM.tps
```
...
évalué sur le terrain) : `models/lda/<nom_du_modèle>/predict/<nom_du_jeu_évalué>/predictions.csv`
...
façons d'y accéder :
- **Interface graphique** (`app/setup_dataset.py`) : galerie filtrable
...
**Interface graphique, en trois outils (`app/setup_dataset.py`,
`app/train_model.py`, `app/predict_dataset.py`).** Contrairement à la
ligne de commande (`tools.pipeline.train_dataset`/`predict_dataset`, qui
relancent chacun tout le landmarking), tout le landmarking a lieu une
seule fois, dans le premier outil -- les deux autres pointent simplement
vers un jeu de données déjà préparé/exporté.

```bash
streamlit run app/setup_dataset.py    # outil 1/3 -- préparer/nettoyer un jeu de données
streamlit run app/train_model.py      # outil 2/3 -- forme de référence + entraînement
streamlit run app/predict_dataset.py  # outil 3/3 -- classification
```

- `app/setup_dataset.py` : un assistant en 6 étapes -- jeu de données
  (racine déjà propre, ou préparation depuis des sources brutes ; les
  codes d'origine `collection_origin` -> `inv_name` peuvent être fournis
  en CSV ou construits depuis un tableau pré-rempli directement dans
  l'appli, voir plus bas) ; paramètres du pipeline (mêmes options que la
  ligne de commande, sous forme de formulaire, avec aperçu de la forme de
  référence GPA choisie) ; détection + recadrage ; validation du recadrage
  (voir ci-dessus) ; placement des landmarks ; validation des landmarks ;
  export. Chaque étape "Run" a un bouton "Skip -- already done" pour
...
- `app/train_model.py` : deux sections indépendantes -- "Build a reference
  shape" (l'équivalent interactif de `landmarks/build_reference.py`, avec
  aperçu de la forme obtenue) et "Train a model" (choix du jeu de données
  déjà exporté, niveau espèce/caste, nom du modèle -- voir le versionnage
  `_v2`/`_v3` décrit à l'étape 4 du scénario 1 plus haut).
- `app/predict_dataset.py` : choix du jeu de données déjà exporté et du
  modèle (liste déroulante, par nom si `--model-name` a été utilisé).

Le champ "Codes d'origine" de `app/setup_dataset.py` (voir
`manifest/origin_table.py`) : si aucun CSV `collection_origin_codes.csv`
n'est fourni, cocher "Build inv_name codes automatically" affiche un
tableau éditable pré-rempli avec les valeurs de la colonne d'origine
trouvées dans le CSV d'identification (une ligne par valeur, `inv_name` à
remplir) ; si cette colonne n'existe pas du tout dans le CSV, un simple
champ texte demande l'`inv_name` à utiliser pour tous les spécimens de
cette source. Le résultat est écrit dans le dossier de sortie de la
source elle-même (`collection_origin_codes.csv`), pas dans un dossier
séparé.
...
- **Les seuils de la distance de Procrustes/du registration cost dans
  `app/setup_dataset.py` (validation des landmarks) sont, comme dans
