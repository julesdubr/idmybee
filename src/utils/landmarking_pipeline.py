"""Shared landmarking + export stages for tools/train_dataset.py and
tools/predict_dataset.py, plus the in-memory single-image counterpart used
...
from tools.export_final_landmarks import main as export_final_landmarks_main
from utils.pipeline_io import dataset_export_dir
...
def add_landmarking_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--detector-model", type=Path, default=DEFAULT_DETECTOR_MODEL,
                         help=f"Wing detector weights: YOLO-OBB .pt for --mode light "
                              f"(default: {DEFAULT_DETECTOR_MODEL}), YOLOE .pt for --mode heavy.")
...
def verbosity_flags(args: argparse.Namespace) -> list[str]:
...
def run_landmarking(args: argparse.Namespace) -> None:
    """Stages 1-4: detection -> crop -> UNet landmarks -> renumbering.

    Writes under the dataset root:
        extraction/<mode>/detection.csv
        extraction/<mode>/images/, crops.csv
        landmarks/landmarks.tps, landmarks.csv
        landmarks/landmarks_numbered.tps, landmarks_numbered.csv
    """
    ...
    verbosity = verbosity_flags(args)

    print(f"\n=== Wing detection ({args.mode}) ===")
    detect_wing_argv = [
        dataset, "--mode", args.mode, "--model", str(args.detector_model),
    ...
    landmarks_predict_argv = [
        ...
        "--n-landmarks", str(args.n_landmarks), *verbosity,
    ]
    if args.device:
...
def run_export(args: argparse.Namespace) -> Path:
    ...
    Writes:
        ...
        biological_data.csv            (photo-level, row-aligned to the TPS)
        failed.csv
    """
    ...
    export_argv = [
        ...
        "--out-height", str(args.out_height), *verbosity,
    ]
    if args.base_dir:
